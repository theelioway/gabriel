...
**Loading Theme Builder into a Browser**::
...
set -U fish_user_paths /home/tim/Dev/timitee/mysqlata $fish_user_paths
set -U fish_user_paths /home/tim/Dev/elioway/elioangels/twig $fish_user_paths
set -U fish_user_paths /home/tim/Dev/elioway/elioangels/generator-art $fish_user_paths
...
```
for R in (npm list -g)
   set N (string replace "├── " "" $R)
   set E (string sub -s 1 -l 8 $N)
   if test (string match -e "@elioway" $E)
       echo "ELIO!" $N
   end
end
```
...
function elioName
  set TITLE (string split ' ' $argv)
  set collected ""
  for s in $TITLE[3..1000]
    set collected $collected$s
  end
  echo $TITLE[1]-$collected-$TITLE[2]In$LOCATION
end
for f in (ls -d */)
   echo elioName $f
end
...
```
for Tag in (ls ../../../../x/)
    set TCase (string upper (string sub --length 1 $Tag))(string lower (string sub --start 2 $Tag))
    echo "<template>
  <$Tag
    v-bind:itemprop=\"field[0]\"
  >{{ field[1] }}</$Tag>
</template>
<script>
export default { props: { "field": Object } }
</script>" > $TCase.vue
end
```
...
```
for file in *.sh
   set len (math (string length "$file") - 3)
   set newName (string sub -l $len "$file")
   mv "$file" "$newName"
end

for f in Exercise_*.png
   set newName (echo "$f" | sed '$s/Exercise_/Exercise/')
   mv $f $newName
end
...
function select
     ...
     mkdir "/home/tim/Dev/timitee/cm_spider/cm_datacycle/exports/$arr[$argv]"
end
...
for f in ls cm_datacycle/retired/exports/*/2020-11*.json
     echo "$f" | string split "/" | select 2 3
end

for c in (docker ps -a)
  set x "$c" | string split "   "
  echo $x[1..10]
end
...
```
indexnext |previous |fish-shell 3.1.2 documentation »
Tutorial
Why fish?
...
vim
Switching to fish?
If you wish to use fish (or any other shell) as your default shell, you need to enter your new shell's executable /usr/local/bin/fish in two places: - add /usr/local/bin/fish to /etc/shells - change your default shell with chsh -s /usr/local/bin/fish
